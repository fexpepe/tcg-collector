...
(function () {
  ...
  function createGradedStore() {
    ...
    const save = () => { data.updatedAt = Date.now(); try { localStorage.setItem(KEY, JSON.stringify(data)); window.TCGShared.marcaSuja(KEY); } catch (e) { window.TCGShared.notifyStorageFull(); } };
    return {
