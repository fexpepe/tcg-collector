...
--
-- As DUAS famílias de whitelist, como na 20260807c (Union Arena):
--   1. card_views + increment_card_view  (corpo mais novo: 20260923a, com a
--      linha do card_views_daily)
--   2. contribute_price                  (corpo da 20260807c)
-- Sem isto, no Dragon Ball Carddass a view de carta e a contribuição de preço
-- são rejeitadas EM SILÊNCIO (as funções só dão `return`).
--
-- Sem cifrão dentro dos corpos de função (lição da 20260923a, 24/09/2026): o
-- editor do Supabase se perde com `$` solto num corpo `$$…$$`. As âncoras de
-- fim das regex usam `\Z`, que no PostgreSQL casa só no fim da string — mesmo
-- efeito do `$` sem quebra de linha no final.
-- ============================================================================
...
alter table public.card_views add constraint card_views_game_check
  check (game = any (array['pokemon','lorcana','onepiece','magic','fab','gundam','dbfw','ygo','digimon','riftbound','unionarena','naruto','hxh','dbc','jump']));

-- increment_card_view: corpo IDÊNTICO ao da 20260923a — só a lista de jogos muda.
...
begin
  ...
  if not (p_game = any (array['pokemon','lorcana','onepiece','magic','fab','gundam','dbfw','ygo','digimon','riftbound','unionarena','naruto','hxh','dbc','jump'])) then return; end if;
...
--
--   # 1) view de carta do Dragon Ball Carddass: 204 e a linha aparece em card_views.
...
--     -d '{"p_game":"dbc","p_card_id":"dbc-h01-1"}'
--
--   curl -s "https://dlnalopazitfdgnmdguu.supabase.co/rest/v1/card_views?game=eq.dbc&select=card_id,views" \
...
--     -H "apikey: sb_publishable_0Qlei5ZvRcEsr18QRdWfGg_N3aR1zyL" -H "Content-Type: application/json" \
--     -d '{"p_game":"dbc","p_card_id":"dbc-h01-1","p_variant":"Normal","p_cond":"NM","p_kind":"listed","p_company":"","p_grade":"","p_value_brl":10}'
-- ============================================================================
